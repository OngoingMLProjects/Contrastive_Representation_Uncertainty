...
def knn_auroc_wilcoxon_repeated_runs_v4(desired_approach = 'Quadratic_typicality', desired_model_type = 'SupCon', baseline_approaches = ['Softmax','Mahalanobis'], baseline_model_types = ['CE','CE'],dataset_type ='grayscale'):
    num_repeats = 8
    ...
    for ID_dataset in all_ID: # Go through the different ID dataset                
        ...
        column_names = copy.deepcopy(baseline_approaches)
...
# Same as before but generalizes to the situation where there are multiple baselines to combine the tables
def multiple_baseline_collated_wilcoxon_post_process_latex_table(df_auroc, df_aupr, df_fpr,caption,label):
