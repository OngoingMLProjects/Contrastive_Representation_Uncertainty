...
def one_dim_typicality_scores_plot():
    ...
    all_ID = ['CIFAR10','CIFAR100','Caltech256','TinyImageNet']
    all_OOD = {'CIFAR10':['MNIST','FashionMNIST','KMNIST'],
    'CIFAR100':['MNIST','FashionMNIST','KMNIST'],
    'Caltech256':['MNIST','FashionMNIST','KMNIST'],
    'TinyImageNet':['MNIST','FashionMNIST','KMNIST']}
    ...
    for Model in Models:# Go through the different models
        for ID in all_ID: # Go through the different ID dataset
            for OOD in all_OOD[ID]: # Go through the different OOD datasets for a particular ID dataset        
                ...
                runs = api.runs(path="nerdk312/evaluation", filters={"config.group":"Baselines Repeats","config.dataset":ID,"config.model_type":Model})
                ...
                for i, run in enumerate(runs): 
                    ...
                    for key in keys:
                        ...
                        with open(read_dir) as f:
                            ...
                            plt.plot(one_dim_score_data[:,0], one_dim_score_data[:,1])
                            plt.show()
