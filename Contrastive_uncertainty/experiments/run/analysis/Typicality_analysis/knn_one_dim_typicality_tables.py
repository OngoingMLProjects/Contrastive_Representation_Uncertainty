...
def knn_table_collated_wilcoxon(desired_approach = 'Quadratic_typicality', desired_model_type = 'SupCon', baseline_approaches = ['Softmax','Mahalanobis'], baseline_model_types = ['CE','CE'],dataset_type ='grayscale',t_test='less',rounding=3):
    ...
    baselines_dict = {'Mahalanobis':{'AUROC':'Mahalanobis AUROC OOD'.lower(),'AUPR':'Mahalanobis AUPR'.lower(),'FPR':'Mahalanobis FPR'.lower()},
                ...
                'Deep Nearest 10 Neighbours':{'AUROC':'Deep Nearest Neighbour - 10 AUROC OOD -'.lower(),'AUPR':'Deep Nearest Neighbour - 10 AUPR OOD -'.lower(),'FPR':'Deep Nearest Neighbour - 10 FPR OOD -'.lower()},

                'Deep Nearest 50 Neighbours':{'AUROC':'Deep Nearest Neighbour - 50 AUROC OOD -'.lower(),'AUPR':'Deep Nearest Neighbour - 50 AUPR OOD -'.lower(),'FPR':'Deep Nearest Neighbour - 50 FPR OOD -'.lower()},

                '1D Typicality CE' :{'AUROC':'Normalized One Dim Class Quadratic Typicality KNN - 10 OOD'.lower(),'AUPR':'Normalized One Dim Class Quadratic Typicality KNN - 10 AUPR'.lower(),'FPR':'Normalized One Dim Class Quadratic Typicality KNN - 10 FPR'.lower()},
                }
    ...
    for approach in baseline_approaches:
        assert approach in baselines_dict, 'No other baselines implemented'
...
if __name__== '__main__':
    ...
    #knn_table_collated_wilcoxon(desired_approach = 'KDE', desired_model_type = 'SupCon', baseline_approaches = ['KDE'], baseline_model_types = ['Moco'],dataset_type ='rgb',t_test='less')
    ...
    # Check with all the different datasets
    knn_table_collated_wilcoxon(desired_approach = 'Quadratic_typicality', desired_model_type = 'SupCon', baseline_approaches = ['Deep Nearest 10 Neighbours','Mahalanobis'], baseline_model_types = ['SupCon','SupCon'],dataset_type ='RGB',t_test='two-sided')