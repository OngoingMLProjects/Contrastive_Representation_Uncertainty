...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, num_channels,latent_dim, block, layers, first_conv=False, maxpool1=False):
        super().__init__()
        ...
        self.layer4 = self._make_layer(block, latent_dim, layers[3], stride=2)
...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self,num_channels, block, layers, latent_dim, input_height, first_conv=False, maxpool1=False):
        super().__init__()
        ...
        # Changed this from 8 to 4 to allow it to allow the input height of MNIST variants to be divisible
        self.upscale_factor = 4

        self.linear = nn.Linear(latent_dim, self.inplanes * 4 * 4)

        self.layer1 = self._make_layer(block, 256, layers[0], scale=1) # Changed the scale from 2 to 1 to take into account the change in the upscale factor
        ...
        self.conv1 = nn.Conv2d(64 * block.expansion, num_channels, kernel_size=3, stride=1, padding=1, bias=False)
...
def resnet18_encoder(num_channels, latent_dim, first_conv, maxpool1):
    return ResNetEncoder(num_channels,latent_dim, EncoderBlock, [2, 2, 2, 2], first_conv, maxpool1)


def resnet18_decoder(num_channels, latent_dim, input_height, first_conv, maxpool1):
    return ResNetDecoder(num_channels, DecoderBlock, [2, 2, 2, 2], latent_dim, input_height, first_conv, maxpool1)


def resnet50_encoder(num_channels, latent_dim, first_conv, maxpool1):
    return ResNetEncoder(num_channels, latent_dim, EncoderBottleneck, [3, 4, 6, 3], first_conv, maxpool1)


def resnet50_decoder(num_channels, latent_dim, input_height, first_conv, maxpool1):
    return ResNetDecoder(num_channels, DecoderBottleneck, [3, 4, 6, 3], latent_dim, input_height, first_conv, maxpool1)
