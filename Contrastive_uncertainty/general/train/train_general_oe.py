############## Used to perform the training for the OE datamodule



from Contrastive_uncertainty.general.datamodules.OE_datamodule import OEDatamodule
import os 
import wandb
...
def train(base_params,trainer_params, model_module,model_function,datamodule_dict):
    ...
    ################## DIFFERENT COMPARED TO THE NORMAL FUNCTION ###############
    auxillary_datamodule = Datamodule_selection(datamodule_dict,config['auxillary_dataset'],config)
    datamodule = OEDatamodule(datamodule,auxillary_datamodule)
    #oe_datamodule = OEDatamodule(datamodule,auxillary_datamodule)
    ###############################################################
    ...
    ############################ Difference compared to the normal function
    model = model_function(model_module,config,datamodule) # TAKE IN OE DATAMODULE RATHER THAN NORMAL DATAMODULE
    ####################
